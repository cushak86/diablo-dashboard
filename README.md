...
**🔗 라이브: https://d2r-dashboard.online** — 설치 없이 브라우저에서 바로 쓰는 디아블로2 레저렉션(D2R) 통합 대시보드입니다. 실시간 공역(테러존) 타이머·음성 알림, 룬 재고로 만들 수 있는 룬워드 판정, 홀리 그레일 체크리스트, 드롭 위치 가이드 등을 제공합니다.

디아블로2 레저렉션(D2R) 통합 대시보드. Next.js 16(App Router) · React 19 기반, 상단 탭 **13개**로 구성됩니다(정본은 lib/pages.js 의 TABS — 이 숫자가 어긋나면 test/nav.test.mjs 가 실패한다).
...
탭 밖 공개 페이지: `/guide`·`/guide/[slug]` — 게임 데이터로 확인한 글(정본 `lib/guides.js`, 2026-08-27). 푸터·홈에서 링크.
