...
/**
 * 좁은 화면(하단 바)은 한 번에 **하나만** 띄운다 — 자리가 없다.
 * 어느 것을 띄울지는 **경로로 정한다.**
 *
 * 왜 랜덤이 아닌가: 서버와 클라이언트가 다른 값을 내면 하이드레이션이 깨진다.
 * Math.random() 도, Date 도 쓸 수 없다. 경로 해시는 양쪽에서 같은 값이 나오고,
 * 페이지마다 다른 상품이 걸려 한 상품만 계속 보이는 일도 없다.
 */
export function pickForPath(list, pathname) {
  let h = 0;
