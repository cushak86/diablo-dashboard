...
export default function TerrorZonePage() {
  ...
  // 마운트 전에만 로딩 셸을 렌더한다(시계 하이드레이션 불일치 방지).
  //
  // 원래는 `!mounted || !data` 였다 — 실데이터가 오기 전 모의로 폴백하면 "모의→실시간" 깜빡임이
  // 생긴다는 이유로 폴백을 미뤘다. 그 대가가 2026-08-09 감사에서 실측됐다:
  // 로딩 셸(min-height 220px)이 실제 2104px 로 바뀌면서 아래 전부가 **1822px 밀렸다**.
  // CLS 0.387 — 임계(0.1)의 3.9배. 모바일 콜드 로드 3회 모두 재현됐다.
  //
  // 이 페이지는 정각 직전에 "지금 어디 열렸나"를 급히 확인하러 오는 곳이다. 0.97초에 첫 화면을
  // 읽기 시작한 손가락이 1.45초에 두 화면 분량 밀린 자리를 누르게 된다.
  // 텍스트가 바뀌는 깜빡임보다 훨씬 비싸다.
  //
  // 모의 상태는 이미 화면에 정직하게 표시된다 — `◈ 모의` / `● 실시간` 모드 칩이 그 용도다.
  // 그래서 폴백으로 완성 레이아웃을 먼저 그리고, 실데이터가 오면 텍스트만 교체한다.
  if (!mounted) {
    return (
      // tz-shell: 로딩 중에도 완성 레이아웃과 **같은 높이를 미리 잡아 둔다**.
      // 이게 없으면 220px 셸이 2138px 로 커지면서 아래 <section class="wrap">(전체 지역 목록)이
      // 통째로 밀린다 — 그게 CLS 0.387 의 정체다(2026-08-09 실측, 임계의 3.9배).
      // 게이트를 `!mounted || !data` 에서 `!mounted` 로 줄이는 것만으로는 부족했다.
      // 시프트가 "API 도착"에서 "하이드레이션"으로 옮겨갔을 뿐 크기는 그대로였다(재측정으로 확인).
      // 서버는 mounted=false 라 항상 셸을 그리므로, 결국 공간을 예약하는 수밖에 없다.
      <main className="tz-shell">
