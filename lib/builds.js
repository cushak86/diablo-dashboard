...
export const BUILDS = [
  {
    ...
    "note": "라이트닝 면역 대응이 최대 관건 → 용병 무한 또는 선더참 필수급. 타격 회복 상충: 맥스롤 최소 32% / 아이시베인즈 86%(4프레임). 시전 속도 68%는 텔포 이동용(단일출처지만 9프레임 브레이크포인트는 일관). 무기는 유니크 투창(타이탄의 복수 등)이라 룬워드 없음.",
    "sources": [
  ...
  },
  {
    ...
    "note": "물리 보우존이라 시전 속도 무관. 신념 룬워드 활(광신 오라)이 최다 인용, 예산형은 부리자/선율. 100% 관통 확보가 클리어 속도 최대 레버. 선율는 예산 활 룬워드(+3 활/석궁 스킬).",
    "sources": [
  ...
  },
  {
    ...
    "skills": [
      {
        "name": "번개 파수병",
        "priority": "1·max",
        "note": "주력 트랩 (설치 비율 번개:죽음 파수병≈4:1)"
      },
      {
        "name": "충격의 거미줄",
        "priority": "2·max(시너지)",
    ...
    "note": "-x% 적 번개저항이 트랩에 적용 → 그리폰의 눈 + 무지개 보석면 다수 채용, 용병 무한으로 후반 폭증. 타격 회복 상충: 맥스롤 48% / 아이시베인즈 86%. 시전 속도 102%는 맥스롤 단일 명시.",
    "sources": [
  ...
  },
  {
    ...
    "note": "모자이크 듀얼 클로가 핵심 — 마무리기로 타격 시 차지 소모 없이 전원소 100% 상시 유지. 공격 속도 기반이라 시전 속도은 텔포 스왑용(~105%)만. 타격 회복 상충: 맥스롤 48% / 아이시베인즈 86%. 방어는 샤코 + 귀 목걸이(물리피해감소).",
    "sources": [
  ...
  },
  {
    ...
    "note": "공격 속도 이득 미미, 회복은 타격 회복 27%만 신경. 물리면역은 용병 노쇠 또는 광폭화(마법)로 처리. 듀얼 웨폰(비탄+야수/맹세 조합)이 표준. 수수께끼 텔포 사용 시 맥스롤은 스왑 63% 시전 속도 권장(상충).",
    "sources": [
  ...
  },
  {
    ...
    "note": "듀얼 웨폰 필수(비탄 + 맹세/법 집행자). 주무기 시전 속도 n/a, 텔포 스왑무기만 63%(맥스롤 단일). 소용돌이보다 기동성↑, 단일 대상 폭딜.",
    "sources": [
  ...
  },
  {
    ...
    "note": "회오리(물리)+폭풍(냉기) 동시 딜로 단일 원소 면역 대응 용이. 시전 속도 99%(스타터)/163%(엔드 9프레임). 타격 회복 상충: 맥스롤 최소 42% / 아이시베인즈 63~99% — 실사용 공통대 60%+. 수수께끼 텔포가 엔드 핵심.",
    "sources": [
  ...
  },
  {
    ...
    "mercenary": "2막 힘 — 냉기 아우라 회피로 시체 보존(시체 폭발용). 무기 통찰 또는 무한",
    "note": "피해증폭→시체 폭발 연쇄로 화면 청소. 스켈+골렘+되살리기로 생존력 최상이라 초보/하드코어 추천. 시전 속도 상충: 맥스롤 125%(엔드) / 아이시베인즈 75%(실용) — 예산형 75% 채택. 용병에 냉기(신성한 결빙)를 피해야 시체가 얼지 않고 남음.",
    "sources": [
  ...
  },
  {
    ...
    "note": "마법 데미지라 원소 면역 무관·관통 우수. 뼈 계열 5스킬 전부 최대 투자(상호 시너지). 스킬 순서 상충: 맥스롤 뼈 창 우선(광역) / 아이시베인즈 뼈 정령 우선(단일) — 최대 투자 집합은 동일. 시전 속도 125%(9프레임): 오크의 심장+거미줄 그물 핵심. 백색는 예산 완드(+뼈창/뼈 스킬).",
    "sources": [
