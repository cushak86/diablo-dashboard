...
// grail:v2 = { version, scope, ids, mirror?, noticeSeen? }. v2가 정본이고 v1은 구버전 호환 투영이다.
const isGrailV2 = (v) =>
  v !== null && typeof v === "object" && !Array.isArray(v) &&
  Number.isInteger(v.version) && v.version >= 1 &&
  ["new", "classic", "all"].includes(v.scope) &&
  isStrArray(v.ids);
...
export const KEYS = [
  // ⚠ key 는 localStorage 키라 절대 불변. label 만 탭 개명(홀리 그레일→아이템(연대기))에 맞춘다.
  { key: "grail:v2", label: "아이템(연대기) 진행", check: isGrailV2, count: (v) => v.ids.length },
  { key: "grail:v1", label: "아이템(연대기) 진행(구버전 호환)", check: isStrArray, count: (v) => v.length },
