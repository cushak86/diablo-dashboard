...
// ⚠ kr(한국어명)의 **출처는 불명이다.** 직역·위키 번역으로 덮어쓰지 말 것.
//   (2026-07-16 이전 주석은 "출처: D2R levels.json koKR 필드"라고 적었으나 **거짓이었다.** 아래가 실측이다.)
//
//   ① levels.json에 koKR 필드는 없다 — 이름 관련 필드는 Name·LevelName·*StringName 3개뿐이고(전체 137필드 중
//      ko/kr/locale 이름의 필드 0개), 한글은 별도 파일 localestrings-kor.json에 **문자열 키**로 들어 있다.
//      (LevelName이 그 키다 — 137/137이 문자열 테이블에 실재함을 확인.)
//   ② 그 문자열 테이블의 값과 여기 kr은 **대부분 다르다** — areas 쌍 68개 중 조인 가능한 45개에서
//      일치 5 · 불일치 40(89%). 예: 여기="핏빛 황무지" vs 덤프="블러드 무어".
//      단 덤프 koKR은 **2.4(2022) 스냅샷**이라 현행 게임과 다를 수 있다 — 어느 쪽이 현재 게임 표기인지는 **미확인**이다.
//   → 즉 여기 kr은 재생성 불가(출처가 없다). **수기 정본으로 보존한다.**
//     사장님이 2026-07-16 19:28에 표기 9건을 확정하셨다(파일 전체를 정본화한 것은 아니다).
//     인게임 실측 확정 진행 중: multi-agent/diablo2-mdb/work/phase1-identity-form/owner-confirm-r1.md
export const TERROR_ZONES = [
  ...
  // 2026-08-09 추가. d2runewizard 가 "Tamoe Highland, Outer Cloister, and The Pit" 로 내보내는데
  // 이 항목이 없어 매칭에 실패했고, 화면에 영문이 그대로 떴다(사장님 제보).
  // 한글명은 지어내지 않고 정본에서 가져왔다 —
  //   diablo2-mdb/mdb/generated/names.json: zone:7 "타모에 고지", zone:27 "수도원 외곽".
  {act:1,kr:"타모에 고지 · 수도원 외곽 · 구덩이",en:"Tamoe Highland, Outer Cloister & The Pit",areas:[["타모에 고지","Tamoe Highland"],["수도원 외곽","Outer Cloister"],["구덩이 1~2층","The Pit Lvl 1-2"]]},
...
/**
 * 영문 지역명 → TERROR_ZONES 매칭.
 *
 * ⚠️ 임계(0.5)는 **질의 토큰 대비 비율**이라 다중 지역 이름에서 희석된다.
 *    실제 사고(2026-08-09): "Tamoe Highland, Outer Cloister, and The Pit" 는 토큰이 6개인데,
 *    그중 한 지역만 아는 후보는 2/6=0.33 이라 임계를 못 넘어 **매칭 실패 → 화면에 영문 노출**.
 *    그래서 전체 이름으로 한 번 보고, 실패하면 쉼표·and 로 **나눠서** 각 조각으로 다시 본다.
 *    조각 하나라도 확실히 맞으면 그 존을 쓴다 — 아무것도 못 맞추는 것보다 낫다.
 */
function bestOver(qt){
...
/** "A, B, and C" / "A & B" 를 조각으로 나눈다. 구분자가 없으면 통째로 하나다. */
...
export function matchZone(apiZoneName){
  ...
  // 통째로 실패했으면 조각으로 나눠 다시 본다. 조각은 토큰이 적어 희석이 없다.
  const parts = splitParts(apiZoneName);
  if(parts.length > 1){
    let pBest=null, pScore=0;
    for(const part of parts){
      const pt = tokens(part);
      if(!pt.length) continue;
      const r = bestOver(pt);
      // 조각은 정확도를 더 요구한다(0.75) — 느슨하면 엉뚱한 존을 잡는다.
      if(r.bestScore >= 0.75 && r.bestScore > pScore){ pScore = r.bestScore; pBest = r.best; }
    }
    if(pBest) return {zone:pBest, score:pScore, partial:true};
  }
  return null;
}
