...
│  ├─ grail/page.js         ✅ 수집 체크리스트 **682종**(신규170 + 클래식512). 범위 토글·이중 진행률.
│  │                        저장은 `grail:v2`(+구버전 호환 투영 `grail:v1`) — lib/grail-store.js가 관리
...
- ~~**홀리 그레일 클래식 확장**~~ ✅ **완료 (2026-07-14).** koKR 표기는 blizzhackers/d2data의 게임 string table
  덤프에서 조인해 확보(위키 번역 미사용). 유니크 385 + 세트 부위 127 = 512 추가 → 총 682.
  기존 사용자 보호: 기본 범위 `new`(화면·진행률 불변) + `grail:v1→v2` 무손실 마이그레이션 + 구버전 롤백 재합류.
  **id는 `lib/grail-classic.js`에 명시된 값이 정본 — 한 글자도 바꾸지 말 것**(바꾸면 사용자 체크가 증발한다).
