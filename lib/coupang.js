...
/**
 * 3시간 슬롯마다 키워드 **2개**(이번 것 + 다음 것) — 상품 풀이 최대 20개라 레일 둘·스트립·하단 바가 겹치지 않는다.
 * 결정적이라(랜덤 아님) 캐시 키가 안정되고, 한 키워드가 두 슬롯에 걸쳐 쓰여 Redis 6시간 캐시와 맞물린다 →
 * 원본 호출은 하루 8회 안팎(2026-08-27 "공격적 노출"로 6h·1개에서 늘렸다. 더 늘리면 호출 제한부터 확인).
 */
export function keywordsFor(now = Date.now()) {
  const i = Math.floor(now / (3 * 3600 * 1000)) % KEYWORDS.length;
  return [KEYWORDS[i], KEYWORDS[(i + 1) % KEYWORDS.length]];
}
